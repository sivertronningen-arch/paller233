...
def init_db():
    if _is_postgres():
        ...
        """)
        # Migrate older installs (if pallets table existed before needs_refill)
        cur.execute("""
        ALTER TABLE pallets
        ...
        # Migrate older installs (if pallets table existed before needs_refill)
        try:
            cur.execute("ALTER TABLE pallets ADD COLUMN needs_refill INTEGER NOT NULL DEFAULT 0")
        except Exception:
            pass
        conn.commit()
        conn.close()
...
def api_get_pallets():
    pallets_rows, _ = db_execute(
        ...
        "SELECT id, name, needs_refill, x, y, w, h FROM pallets ORDER BY id",
        fetch=True
    )
    out = []
    for p in pallets_rows:
        ...
        out.append({
            ...
            "needs_refill": bool((p[2] if isinstance(p, tuple) else p["needs_refill"])) ,
            "x": (p[3] if isinstance(p, tuple) else p["x"]),
            "y": (p[4] if isinstance(p, tuple) else p["y"]),
            "w": (p[5] if isinstance(p, tuple) else p["w"]),
            "h": (p[6] if isinstance(p, tuple) else p["h"]),
            "articles": arts
        })
    return jsonify(out)
...
@app.put("/api/pallets/<int:pallet_id>")
def api_update_pallet(pallet_id: int):
    """Update pallet metadata (custom name + needs_refill flag)."""
    data = request.get_json(force=True, silent=True) or {}
    name = data.get("name", None)
    needs_refill = data.get("needs_refill", None)

    rows, _ = db_execute(
        "SELECT id, name, needs_refill FROM pallets WHERE id=?",
        "SELECT id, name, needs_refill FROM pallets WHERE id=%s",
        (pallet_id,),
        fetch=True,
    )
    if not rows:
    ...
    current = rows[0]
    cur_name = current[1] if isinstance(current, tuple) else current["name"]
    cur_refill = current[2] if isinstance(current, tuple) else current["needs_refill"]
    ...
    new_refill = bool(cur_refill)
    if needs_refill is not None:
        new_refill = bool(needs_refill)

    if _is_postgres():
        db_execute(
            "UPDATE pallets SET name=?, needs_refill=? WHERE id=?",  # unused
            "UPDATE pallets SET name=%s, needs_refill=%s WHERE id=%s",
            (new_name, new_refill, pallet_id),
        )
    else:
        db_execute(
            "UPDATE pallets SET name=?, needs_refill=? WHERE id=?",
            "UPDATE pallets SET name=%s, needs_refill=%s WHERE id=%s",  # unused
            (new_name, 1 if new_refill else 0, pallet_id),
        )
    db_commit()
    return jsonify({"ok": True, "id": pallet_id, "name": new_name, "needs_refill": new_refill})
