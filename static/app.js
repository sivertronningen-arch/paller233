...
function setServerConnected(ok){
  serverConnected = !!ok;
  if (!connBanner) return;
  connBanner.hidden = serverConnected;
}
...
async function serverLoad() {
  try {
    ...
    if (!r.ok) { setServerConnected(false); return null; }
    const j = await r.json();
    if (!j || j.ok !== true) { setServerConnected(false); return null; }
    setServerConnected(true);
    return j.state || null;
  } catch { setServerConnected(false); return null; }
}
async function serverSave(state) {
  try {
    ...
    if (!r.ok) { setServerConnected(false); return false; }
    const j = await r.json().catch(() => null);
    const ok = !!(j && j.ok === true);
    setServerConnected(ok);
    return ok;
  } catch { setServerConnected(false); return false; }
}
function localLoad() {
...
function debounceAutoSave() {
  ...
  // UX: show that we are syncing in the background
  if (serverConnected) setStatus("Synkroniserer…");
...
async function saveAll({ quiet }) {
  ...
  setStatus(ok ? "Lagret" : "Ikke tilkoblet server");
...
function makePallet(x, y, overrides = {}) {
  return { id: uid(), kind:"pallet", x, y, rot:0, scale:1, name: nextPalletName(), article:"", low:false, note:"", ...overrides };
}
function makeFirer(x, y) {
  const gapX = 85, gapY = 60;
  const children = [
    { id: uid(), rx: -gapX/2, ry: -gapY/2, name: nextPalletName(), article:"", low:false, note:"" },
    { id: uid(), rx:  gapX/2, ry: -gapY/2, name: nextPalletName(), article:"", low:false, note:"" },
    { id: uid(), rx: -gapX/2, ry:  gapY/2, name: nextPalletName(), article:"", low:false, note:"" },
    { id: uid(), rx:  gapX/2, ry:  gapY/2, name: nextPalletName(), article:"", low:false, note:"" },
  ];
...
function makeEndegavel(x, y) {
  return { id: uid(), kind:"endegavel", x, y, rot:0, scale:1, name:"Endegavel",
    top:{ id: uid(), name:"Overprodukt", article:"", low:false, note:"" },
    bottom:{ id: uid(), name: nextPalletName(), article:"", low:false, note:"" },
  };
}
...
function setSelected(sel){
  selected = sel;
  // Prevent accidental deletion while editing a sub-pallet inside a group
  btnDelete.disabled = !selected || !!(selected && selected.subId);
  renderPanel();
  // Mobile: show/hide side panel based on selection
  if (isMobile()) {
    if (selected) openPanel();
    else closePanel();
    // Hide the top menu while working with the map
    closeMenu();
  }
  markDirty();
}
  ...
  for (let i=items.length-1; i>=0; i--) {
    ...
    if (it.kind === "pallet") {
      ...
      for (const ch of it.children) {
        if (hitTestPalletLocal(p.x - ch.rx, p.y - ch.ry)) return { itemId: it.id, subId: ch.id, subKind:"child" };
      }
    } else if (it.kind === "endegavel") {
      const p = invPoint(it, wx, wy);
      if (hitTestPalletLocal(p.x, p.y + 70)) return { itemId: it.id, subId: it.top.id, subKind:"top" };
      if (hitTestPalletLocal(p.x, p.y)) return { itemId: it.id, subId: it.bottom.id, subKind:"bottom" };
    }
  }
  return null;
}
...
canvas.addEventListener("pointerdown", (e) => {
  ...
  // Desktop UX: drag empty space to pan (no need for zooming)
...
function renderPanel(){
  ...
  if (it.kind === "firer") {
    // If a sub-pallet is selected, edit ONLY that pallet
    if (selected.subId) {
      const ch = (it.children || []).find(x => x.id === selected.subId);
      panelBody.appendChild(el("div", { class:"muted", html:`Firer-torg: <b>${(it.name||"Firer-torg")}</b>` }));

      panelBody.appendChild(el("div", { class:"row" }, [
        el("button", { class:"btn", onclick: () => setSelected({ itemId: it.id }) }, [document.createTextNode("Rediger torget")]),
      ]));
      panelBody.appendChild(el("div", { class:"hr" }));

      if (!ch) {
        panelBody.appendChild(el("div", { class:"muted", html:"Fant ikke pallen." }));
        return;
      }
      ...
      note.value = ch.note || "";
      note.addEventListener("input", () => { ch.note = note.value; markDirty(); debounceAutoSave(); });
      panelBody.appendChild(field("Plan / kommentar", note));
    ...
    panelBody.appendChild(el("div", { class:"muted", html:"Tips: Trykk på en av pallene i torget for å redigere kun den pallen." }));
    panelBody.appendChild(el("div", { class:"hr" }));

    for (let i=0;i<(it.children||[]).length;i++) {
      const ch = it.children[i];
      panelBody.appendChild(el("div", { class:"row" }, [
        el("button", { class:"btn", onclick: () => setSelected({ itemId: it.id, subId: ch.id, subKind:"child" }) }, [document.createTextNode(`Rediger Pall ${i+1}`)]),
      ]));
    }

    // Allow deleting the whole group here
    panelBody.appendChild(el("div", { class:"hr" }));
    panelBody.appendChild(el("button", { class:"btn btn--danger", onclick: () => {
      if (!confirm("Slette hele firer-torget?") ) return;
      items = items.filter(x => x.id !== it.id);
      setSelected(null);
      debounceAutoSave();
    } }, [document.createTextNode("Slett firer-torg")]) );
    return;
  }

  if (it.kind === "endegavel") {
    // If a part is selected, edit ONLY that part
    if (selected.subId) {
      const part = (it.top && it.top.id === selected.subId) ? it.top
        : (it.bottom && it.bottom.id === selected.subId) ? it.bottom
        : null;

      panelBody.appendChild(el("div", { class:"muted", html:`Endegavel: <b>${(it.name||"Endegavel")}</b>` }));
      panelBody.appendChild(el("div", { class:"row" }, [
        el("button", { class:"btn", onclick: () => setSelected({ itemId: it.id }) }, [document.createTextNode("Rediger endegavel")]),
      ]));
      panelBody.appendChild(el("div", { class:"hr" }));

      if (!part) {
        panelBody.appendChild(el("div", { class:"muted", html:"Fant ikke del." }));
        return;
      }
      const isTop = (it.top && it.top.id === selected.subId);
      panelBody.appendChild(el("div", { class:"label", html: isTop ? "Overprodukt" : "Pall" }));

      const inpName = el("input", { class:"input", value: part.name || "" });
      inpName.addEventListener("input", () => { part.name = inpName.value; markDirty(); debounceAutoSave(); });
      panelBody.appendChild(field("Navn", inpName));

      const inpArt = el("input", { class:"input", value: part.article || "" });
      inpArt.addEventListener("input", () => { part.article = inpArt.value; updateSearchMatches(); markDirty(); debounceAutoSave(); });
      panelBody.appendChild(field("Artikkelnummer (søk)", inpArt));

      const note = el("textarea", { class:"input", rows:"3" });
      note.value = part.note || "";
      note.addEventListener("input", () => { part.note = note.value; markDirty(); debounceAutoSave(); });
      panelBody.appendChild(field("Plan / kommentar", note));

      panelBody.appendChild(checkbox("Lav fyllingsgrad (blir rød)", part.low, (v) => { part.low = v; markDirty(); debounceAutoSave(); }));
      return;
    }
    ...
    panelBody.appendChild(el("div", { class:"muted", html:"Tips: Trykk på overproduktet eller pallen i endegavlen for å redigere kun den delen." }));
    panelBody.appendChild(el("div", { class:"hr" }));
    panelBody.appendChild(el("div", { class:"row" }, [
      el("button", { class:"btn", onclick: () => setSelected({ itemId: it.id, subId: it.top.id, subKind:"top" }) }, [document.createTextNode("Rediger overprodukt")]),
      el("button", { class:"btn", onclick: () => setSelected({ itemId: it.id, subId: it.bottom.id, subKind:"bottom" }) }, [document.createTextNode("Rediger pall")]),
    ]));

    panelBody.appendChild(el("div", { class:"hr" }));
    panelBody.appendChild(el("button", { class:"btn btn--danger", onclick: () => {
      if (!confirm("Slette hele endegavlen?") ) return;
      items = items.filter(x => x.id !== it.id);
      setSelected(null);
      debounceAutoSave();
    } }, [document.createTextNode("Slett endegavel")]) );
    return;
  }
  ...
  if (it.kind === "pallet") {
    ...
    const note = el("textarea", { class:"input", rows:"3" });
    note.value = it.note || "";
    note.addEventListener("input", () => { it.note = note.value; markDirty(); debounceAutoSave(); });
    panelBody.appendChild(field("Plan / kommentar", note));
