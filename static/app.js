...
const palletNameInput = document.getElementById("palletName");
const needsRefillCb = document.getElementById("needsRefill");
...
function clearSelectionUI(){
  ...
  palletNameInput.disabled = true;
  needsRefillCb.checked = false;
  needsRefillCb.disabled = true;
  articlesArea.value = "";
...
function enableEditor(pallet){
  ...
  palletNameInput.disabled = false;
  palletNameInput.value = pallet.name;
  needsRefillCb.disabled = false;
  needsRefillCb.checked = !!pallet.needs_refill;
...
saveBtn.addEventListener("click", async ()=>{
  ...
  try{
    // Update metadata first (name + refill flag)
    await apiSend(`/api/pallets/${selectedId}`, "PUT", {
      name: palletNameInput.value.trim() || undefined,
      needs_refill: !!needsRefillCb.checked
    });
...
cancelBtn.addEventListener("click", ()=>{
  ...
  if(!p) return;
  palletNameInput.value = p.name;
  needsRefillCb.checked = !!p.needs_refill;
  articlesArea.value = (p.articles || []).join("\n");
});

// Quick toggle: mark needs refill and update immediately
needsRefillCb.addEventListener("change", async ()=>{
  if(!selectedId) return;
  try{
    await apiSend(`/api/pallets/${selectedId}`, "PUT", {
      name: palletNameInput.value.trim() || undefined,
