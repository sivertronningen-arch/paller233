...
canvas.addEventListener("pointerdown", (e) => {
  ...
  if (isTouch(e)) {
    ...
    if (touchCount() === 2) {
      ...
      touch.lastCenter = center2(pts[0], pts[1]);
    } else {
  ...
  pointer.dragging = (isMobile() ? false : !!hit);
});
...
canvas.addEventListener("pointermove", (e) => {
  ...
  if (isTouch(e)) {
    ...
    if (touchCount() >= 2) {
      ...
      // Also pan by moving the center
      if (touch.lastCenter) {
        panBy(c.x - touch.lastCenter.x, c.y - touch.lastCenter.y);
      }
      zoomAboutScreenPoint(newZoom, c.x, c.y);
      touch.lastCenter = c;
      touch.moved = true;
      return;
    }
