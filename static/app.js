...
canvas.addEventListener("pointermove", (e) => {
  if (!pointer.down && !(isTouch(e) && touch.active.has(e.pointerId))) return;
  const rect = canvas.getBoundingClientRect();
  const sx = e.clientX - rect.left;
  const sy = e.clientY - rect.top;
...
// If pinch ended return to pan (Google Maps-like)
if (touch.isPinching && touchCount() === 1) {
  ...
  touch.startZoom = view.zoom;
  // set pan base to the remaining finger
  const only = Array.from(touch.active.values())[0];
  touch.tapStart = { x: only.x, y: only.y };
  touch.lastCenter = { x: only.x, y: only.y };
}
...
canvas.addEventListener("pointerup", (e) => {
  // For touch we may still have other fingers down
  if (!pointer.down && !isTouch(e)) return;
  if (!isTouch(e)) pointer.down = false;
  ...
  if (isTouch(e)) {
    ...
    if (touchCount() === 0) pointer.down = false;
    ...
    if (touchCount() < 2) {
      ...
      touch.startCenter = null;
      touch.startWorld = null;
      // if one finger is still down, make pan continue smoothly
      if (touchCount() === 1) {
        const only = Array.from(touch.active.values())[0];
        touch.tapStart = { x: only.x, y: only.y };
        touch.lastCenter = { x: only.x, y: only.y };
...
function renderPanel(){
  panelBody.innerHTML = "";
  if (!selected) {
    panelBody.appendChild(el("div", { class:"muted", html:"Klikk på en pall for å redigere." }));
    return;
  }
  const it = findItemById(selected.itemId);
  if (!it) return;
  ...
  function renderPalletEditor(obj){
    const inpArt = el("input", { class:"input", value: obj.article || "", inputmode:"numeric", placeholder:"Artikkelnummer" });
    inpArt.addEventListener("input", () => { obj.article = inpArt.value; updateSearchMatches?.(); markDirty(); debounceAutoSave(); render(); });
    panelBody.appendChild(field("Artikkelnummer", inpArt));

    panelBody.appendChild(checkbox("Lav fyllingsgrad", !!obj.low, (v) => { obj.low = v; markDirty(); debounceAutoSave(); render(); }));

    const note = el("textarea", { class:"input", rows: 2, placeholder:"Kommentar / plan", value: obj.note || "" });
    note.addEventListener("input", () => { obj.note = note.value; markDirty(); debounceAutoSave(); });
    panelBody.appendChild(field("Kommentar", note));
  }

  // Desktop: keep transform controls for groups too
  if (!mobile) {
    ...
    panelBody.appendChild(el("div", { class:"row" }, [
      el("button", { class:"btn", onclick: () => { it.rot -= Math.PI/12; markDirty(); debounceAutoSave(); render(); renderPanel(); } }, [document.createTextNode("⟲ Roter")]),
      el("button", { class:"btn", onclick: () => { it.rot += Math.PI/12; markDirty(); debounceAutoSave(); render(); renderPanel(); } }, [document.createTextNode("Roter ⟳")]),
      el("button", { class:"btn", onclick: () => { it.scale = clamp(it.scale*0.92, 0.35, 3.0); markDirty(); debounceAutoSave(); render(); renderPanel(); } }, [document.createTextNode("Mindre -")]),
      el("button", { class:"btn", onclick: () => { it.scale = clamp(it.scale*1.08, 0.35, 3.0); markDirty(); debounceAutoSave(); render(); renderPanel(); } }, [document.createTextNode("Større +")]),
      el("button", { class:"btn", onclick: () => { it.rot = 0; it.scale = 1; markDirty(); debounceAutoSave(); render(); renderPanel(); } }, [document.createTextNode("Nullstill")]),
    ]));
  ...
  if (it.kind === "pallet") {
    if (!mobile) {
      ...
      inpName.addEventListener("input", () => { it.name = inpName.value; markDirty(); debounceAutoSave(); render(); });
      panelBody.appendChild(field("Navn", inpName));
    }
    renderPalletEditor(it);
    if (mobile) panelBody.scrollTop = 0;
    return;
  }

  if (it.kind === "firer") {
    // show/allow group name on desktop (name shown over group on map)
    if (!mobile) {
      const nameInp = el("input", { class:"input", value: it.name || "" });
      nameInp.addEventListener("input", () => { it.name = nameInp.value; markDirty(); debounceAutoSave(); render(); });
      panelBody.appendChild(field("Navn på firer-torg", nameInp));
      panelBody.appendChild(el("div", { class:"muted", html:"Klikk på en pall i firer-torget for å redigere kun den pallen." }));
      panelBody.appendChild(el("div", { class:"hr" }));
    }
    const ch = it.children.find(x => x.id === selected.subId) || it.children[0];
    panelBody.appendChild(el("div", { class:"label", html: ch.name || "Pall" }));
    if (!mobile) {
      const inpName = el("input", { class:"input", value: ch.name || "" });
      inpName.addEventListener("input", () => { ch.name = inpName.value; markDirty(); debounceAutoSave(); render(); });
      panelBody.appendChild(field("Navn", inpName));
    }
    renderPalletEditor(ch);
    if (mobile) panelBody.scrollTop = 0;
    return;
  }
  ...
  if (it.kind === "endegavel") {
    if (!mobile) {
      const nameInp = el("input", { class:"input", value: it.name || "" });
      nameInp.addEventListener("input", () => { it.name = nameInp.value; markDirty(); debounceAutoSave(); render(); });
      panelBody.appendChild(field("Navn på endegavel", nameInp));
      panelBody.appendChild(el("div", { class:"muted", html:"Klikk på overprodukt eller pall for å redigere kun den delen." }));
      panelBody.appendChild(el("div", { class:"hr" }));
    }
    let obj = it.bottom;
    let label = "Pall";
    if (selected.subId === it.top.id) { obj = it.top; label = "Overprodukt"; }
    if (selected.subId === it.bottom.id) { obj = it.bottom; label = "Pall"; }
    panelBody.appendChild(el("div", { class:"label", html: label }));
    if (!mobile) {
      const inpName = el("input", { class:"input", value: obj.name || "" });
      inpName.addEventListener("input", () => { obj.name = inpName.value; markDirty(); debounceAutoSave(); render(); });
      panelBody.appendChild(field("Navn", inpName));
    }
    renderPalletEditor(obj);
    if (mobile) panelBody.scrollTop = 0;
    return;
  }
}
